...
#     print("Prediction: ", text_clf.predict(["bad movie stupid story", "very nice movie wonderful character",
#                                             "slow pace at the beginning but a suprising ending, quite a nice movie"]))
#
#     return AUC
#
#
...
#     # train model using pipeline (tokenizer => tf-idf transformer => linear SVM classifier)
#     text_clf = Pipeline([('vect', CountVectorizer()), ('tfidf', TfidfTransformer()),('clf', SGDClassifier(loss='hinge', penalty='l2', alpha=1e-3, n_iter_no_change=5, random_state=42))])
#     _ = text_clf.fit(train_features, train_labels)
#
#     # evaluate on test set
#     predicted = text_clf.predict(test_features)
...
def RF(train_features, train_labels, test_features, test_labels):
    ...
    return metrics.classification_report(test_labels, predicted,output_dict= True)
