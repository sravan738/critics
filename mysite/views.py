...
from mysite import sanpleRF

database = "C:\\Users\\siva\\project1\\db.sqlite3"
...
def classification(request):
    return render(request, 'mysite/classification.html')
...
def token001(request):
    ...
    if request.method == 'POST' and request.FILES['csvfile']:  # If the form has been submitted...
        ...
        response = tokens.normalization(data)
    else:
        print("no file selected")
    forest = sanpleRF.mm()
    print(response)
    print(forest)
    context = {'data': response, 'RF': forest}
...
def token003(request):
    ...
    if request.method == 'POST':  # If the form has been submitted...
        ...
        visualize_obj = dv.Visualize(data)
