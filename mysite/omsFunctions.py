...
def preProcessing2(inputFileStr,printResult):
...
def tokenizeReviews1(inputFileStr,printResult):
    tokenizedReviews={}
    inputFile = inputFileStr
    #outputFile=open (outputFileStr,"w")
    tokenizer = nltk.tokenize.punkt.PunktSentenceTokenizer()
...
def posTagging1(inputFileStr,printResult):
    inputFile =inputFileStr
    #outputFile=open (outputFileStr,"w")
    inputTupples=ast.literal_eval(inputFile)
    print(inputTupples)
...
def aspectExtraction1(inputFileStr,printResult):
    inputFile =inputFileStr
    #outputFile=open (outputFileStr,"w")
    inputTupples=ast.literal_eval(inputFile)
    prevWord=''
...
def identifyOpinionWords1(inputReviewListStr,inputAspectListStr,printResult):
    inputReviewList = inputReviewListStr
    inputAspectList = inputAspectListStr
    #outputAspectOpinionList = outputAspectOpinionListStr
    inputReviewsTuples = ast.literal_eval(inputReviewList)
    inputAspectTuples = ast.literal_eval(inputAspectList)
    outputAspectOpinionTuples={}
    orientationCache={}
    ...
    for aspect,no in inputAspectTuples:
        ...
        if(count>0):
            ...
            print(aspect,':\t\tPositive => ', outputAspectOpinionTuples[aspect][0], '\tNegative => ',outputAspectOpinionTuples[aspect][1])
    if(printResult):
        print(outputAspectOpinionTuples)
